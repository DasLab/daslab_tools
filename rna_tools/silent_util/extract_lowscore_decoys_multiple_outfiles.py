...
parser = argparse.ArgumentParser(
    description='Extract N lowest-scoring models from each silent file into separate .topN.out files.',
    epilog=(
        'The last positional args set the score column and count:\n'
        '  <script> silent.out 10       — extract 10 lowest by score\n'
        '  <script> silent.out -rms 5   — extract 5 lowest by rms\n'
        '  <script> silent.out +rms 5   — extract 5 highest by rms\n'
        '  <script> silent.out 2.5      — extract all with score < 2.5 (float = cutoff)'
    ),
